...
void LT738_SDRAM_initail(uint8_t mclk)
{
	uint16_t sdram_itv;
...
void LT738_DMA_24bit_Block
(
 uint8_t SCS         // 选择外挂的SPI   : SCS：0       SCS：1
,uint8_t Clk         // SPI时钟分频参数 : SPI Clock = System Clock /{(Clk+1)*2}
,uint16_t X1         // 传输到内存X1的位置
,uint16_t Y1         // 传输到内存Y1的位置
,uint16_t X_W        // DMA传输数据的宽度
,uint16_t Y_H        // DMA传输数据的高度
,uint16_t P_W        // 图片的宽度
...
void LT738_PWM0_Duty(uint16_t Compare_Buffer)
{
	Set_Timer0_Compare_Buffer(Compare_Buffer);
}
...
void Show(void)
{
  ...
  Active_Window_XY(0,0);
	Active_Window_WH(LCD_XSIZE_TFT,LCD_YSIZE_TFT);			
	LT738_DrawSquare_Fill(0,0,1023,599,Black);
	
	Canvas_Image_Start_address(layer2_start_addr);
	Canvas_image_width(LCD_XSIZE_TFT);
  Active_Window_XY(0,0);
	Active_Window_WH(LCD_XSIZE_TFT,LCD_YSIZE_TFT);			
	LT738_DrawSquare_Fill(0,0,1023,599,Blue2);
  ...
  Active_Window_XY(0,0);
	Active_Window_WH(LCD_XSIZE_TFT,LCD_YSIZE_TFT);			
	LT738_DrawSquare_Fill(0,0,LCD_XSIZE_TFT-1,LCD_YSIZE_TFT-1,Blue3);
	...
	LT738_DrawSquare_Fill(600,0,1020,320,Blue2);
	for (i=0;i<3;i++)
	    {
			 for (j=0;j<4;j++)
				   {
  ...
  Active_Window_XY(0,0);
	Active_Window_WH(LCD_XSIZE_TFT,LCD_YSIZE_TFT);			
	LT738_DrawSquare_Fill(0,0,LCD_XSIZE_TFT-1,LCD_YSIZE_TFT-1,0);
}
