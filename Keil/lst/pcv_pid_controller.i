...
void PCV_Reset(void) {
    ...
    float target = 20.0f; 
    float internal_target = target - 1.2f; 
...
void PCV_PrepareInput(PCV_Input_t *input) {
    input->Current_Pressure = (float)Vp;  
    input->Current_Flow     = 0.0f;       

    input->Target_PIP  = 20.0f; 
    input->Target_PEEP = 2.3f;  
    input->RR_bpm      = 12.0f;     
...
void PCV_OutputToHardware(const PCV_Output_t *output) {
    ...
    uint32_t pwm_insp = (uint32_t)(output->Insp_Valve_Output / 100.0f * 3000);

    
    ((((EPWM_T *) (((uint32_t)0x40000000) + 0x58000UL)))->CMPDAT[(4)]= (pwm_insp));
}
